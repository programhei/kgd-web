...
const emit = defineEmits<{
  'update:updateOpened': [value: boolean],
...
const [ScaleModal, scaleModalApi] = useVbenModal({
  ...
  onOpened() {
    ...
    Object.assign(modelRef, {
      ...props.userInfo,
      sex: Number(props.userInfo.sex),
      status: Number(props.userInfo.status),
    })
    // 角色下拉选择
  ...
  onConfirm() {
    modal.confirm({
      ...
      onOk() {
        validate()
          .then(() => {
            emit('update:updateUser', toRaw(modelRef))
          })
          .catch(err => {
            console.log('error', err);
          })
          .finally(() => {
            emit('update:updateOpened', false)
          });
      },
      onCancel() {
        message.info('用户取消操作')
...
<template>
  <div class="space-y-4">
    <ScaleModal title="修改用户" class="w-[550px] bg-background-deep">
