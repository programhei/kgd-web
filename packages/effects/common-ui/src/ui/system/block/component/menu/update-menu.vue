...
const [ScaleModal, scaleModalApi] = useVbenModal({
  ...
  onOpened() {
    ...
    Object.assign(modelRef, {
      ...props.menuInfo,
      status: Number(props.menuInfo.status),
      isFrame: Number(props.menuInfo.isFrame),
    })
  },
  onCancel() {
  ...
  onConfirm() {
    modal.confirm({
      ...
      onOk() {
        validate()
          .then(() => {
            emit('update:updateMenu', toRaw(modelRef))
          })
          .catch(err => {
...
<template>
  <div class="space-y-4">
    <ScaleModal title="编辑菜单" class="w-[550px] bg-background-deep">
