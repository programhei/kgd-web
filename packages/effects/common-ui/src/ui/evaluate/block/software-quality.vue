...
import {
  ClearOutlined,
  DatabaseOutlined,
  DeleteOutlined, DownloadOutlined, ExclamationOutlined
...
import {ViewRJDataModal, ViewTXDataModal} from './component'
    ...
    if (dataList.length === 0) {
      ...
      dataList.forEach((dataItem: any, index: number) => {
        ...
        result.push({
          ...
          timeResponseCharact: getFieldValue('timeResponseCharact', 'time_response_charact'),
          resourceUtilization: getFieldValue('resourceUtilization', 'resource_utilization'),
          calculationCycle: getFieldValue('calculationCycle', 'calculation_cycle'),
          memoryUsage: getFieldValue('memoryUsage', 'memory_usage'),
          codeCommentRate: getFieldValue('codeCommentRate', 'code_comment_rate'),
          codeStandardRate: getFieldValue('codeStandardRate', 'code_standard_rate'),
          continuousRunTime: getFieldValue('continuousRunTime', 'continuous_run_time'),
          interfaceFaultTolerance: getFieldValue('interfaceFaultTolerance', 'interface_fault_tolerance'),
        })
      })
    }
  })
...
const columns = ref([
  {
    title: '时间响应特性',
    dataIndex: 'timeResponseCharact',
    key: 'timeResponseCharact',
    width: 120,
    align: 'center',
    ellipsis: true,
  },
  ...
  {
    title: '代码规范率(%)',
    dataIndex: 'codeStandardRate',
    key: 'codeStandardRate',
    width: 130,
    align: 'center',
    ellipsis: true,
  },
...
const scoreTableData = computed(() => {
  ...
  const scores = props.evaluateScore[0]['scores'];
  ...
  const processTree = (node: any, level: number = 0): any[] => {
    ...
    // 添加当前节点
    // 如果 name 为 null，使用 scoreType 作为名称
    const displayName = node.name || node.scoreType || '';
    if (displayName) {
      result.push({
        name: displayName,
        score: node.score ?? 0,
