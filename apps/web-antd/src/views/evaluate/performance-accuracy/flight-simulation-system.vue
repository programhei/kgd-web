...
import {FlightSimulationSystem} from '@vben/common-ui'
import {ref} from "vue";
...
const handleLoadData = (value: any) => {
  datasource.value = []
  list({extra: 'FlightXN', isStandard: value}).then(res => {
...
const handleDeleteData = (value) => {
  deleteData({...value, extra: 'FlightXN'}).then(res => {
...
const handleSubmitUpload = (value: any) => {
  ...
  upload(formData).then(res => {
      ...
      importData({filePath: res.path, extra: 'FlightXN', isStandard, isPerformance}).then(res => {
