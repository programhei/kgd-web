...
import {RadarSimulationSystem} from '@vben/common-ui'
import {ref} from "vue";
...
const handleLoadData = (value: any) => {
  datasource.value = []
  list({extra: 'RadarXN', isStandard: value}).then(res => {
...
const handleSubmitUpload = (value: any) => {
  ...
  upload(formData).then(res => {
      ...
      importData({filePath: res.path, extra: 'RadarXN', isStandard, isPerformance}).then(res => {
