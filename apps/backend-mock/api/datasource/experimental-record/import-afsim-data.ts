...
// 导入实验数据
function importAfSimData(event: H3Event<EventHandlerRequest>, path: any) {
  return request({
    url: '/datasource/experimentalRecord/importAfSimData',
    method: 'get',
    params: {path},
    headers: {
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const res: any = await importAfSimData(event, filePath)
