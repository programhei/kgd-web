...
function delExperimentalIdCorrespondence(event: H3Event<EventHandlerRequest>, id: number) {
  return request({
    url: '/datasource/experimentalIdCorrespondence/' + id,
    method: 'delete',
    headers: {
...
export default defineEventHandler(async (event) => {
  ...
  const res: any = await delExperimentalIdCorrespondence(event, body.id)
